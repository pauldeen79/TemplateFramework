...
public class MultipleStringContentBuilderTemplateRenderer : ITemplateRenderer
{
    ...
    public async Task<Result> RenderAsync(ITemplateEngineContext context, CancellationToken token)
    {
        ...
        if (multipleContentBuilderTemplate is not null)
        {
            ...
            return await multipleContentBuilderTemplate.RenderAsync(multipleContentBuilder, token).ConfigureAwait(false);
        }
        ...
        var result = await context.Engine.RenderAsync(singleRequest, token).ConfigureAwait(false);
